...
from monetary_rl.phase11_extreme_specs import NEW_ENV_SPECS, make_model
...
MATRIX_DIR = ROOT / "outputs" / "phase11" / "extreme_matrix"
OUTPUT_DIR = ROOT / "outputs" / "phase11" / "extreme_numerical_compare"
...
def main() -> None:
    ...
    for idx, case_spec in enumerate(NEW_ENV_SPECS):
        ...
        rng = np.random.default_rng(20280400 + idx)
    ...
    lines = []
    lines.append("# Phase 11 新增两档数值求解对照")
    lines.append("")
    lines.append("## 总表")
    lines.append("")
    ...
    lines.append("## 结论")
    lines.append("")
    lines.append("- 本表只覆盖 `phase11` 新增的两档 × 三类环境。")
    lines.append("- 若 `dp_improvement_vs_riccati_pct` 为正，则说明新增扭曲下传统 benchmark Riccati 外推已被环境内数值解压过。")
