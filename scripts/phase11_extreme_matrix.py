...
from monetary_rl.phase11_extreme_specs import (
...
OUTPUT_DIR = ROOT / "outputs" / "phase11" / "extreme_matrix"
...
def run_rl_one(spec: dict, algo: str, seed: int) -> dict:
    ...
    rng = np.random.default_rng(20270405 + seed)
...
def run_baselines_for_env(spec: dict, riccati_policy, riccati_k: np.ndarray) -> list[dict]:
    ...
    if linear_cache.exists():
        ...
        rng = np.random.default_rng(20270500)
    ...
    for name, policy_fn in baseline_specs:
        ...
        if path.exists():
            ...
            rng = np.random.default_rng(20270600 + seed_map[name])
            traj = simulate_with_common_shocks(
                model,
                {name: policy_fn},
                initial_state=np.asarray(spec["common_initial_state"], dtype=float),
...
def write_summaries(rl_df: pd.DataFrame, baseline_df: pd.DataFrame, coeff_df: pd.DataFrame, training_df: pd.DataFrame) -> None:
    ...
    phase7_rl_summary = pd.read_csv(PHASE7_DIR / "rl_summary.csv")
    combined_rl = pd.concat([phase7_rl_summary, rl_summary], ignore_index=True, sort=False)
    tier_cat = pd.CategoricalDtype(categories=TIER_ORDER, ordered=True)
    combined_rl["tier"] = combined_rl["tier"].astype(tier_cat)
    combined_rl = combined_rl.sort_values(["group", "tier", "algo"]).reset_index(drop=True)
    combined_rl.to_csv(OUTPUT_DIR / "combined_rl_summary.csv", index=False)

    phase7_all_policy = pd.read_csv(PHASE7_DIR / "all_policy_summary.csv")
    combined_all_policy = pd.concat([phase7_all_policy, all_policy_df], ignore_index=True, sort=False)
    combined_all_policy["tier"] = combined_all_policy["tier"].astype(tier_cat)
    combined_all_policy = combined_all_policy.sort_values(["group", "tier", "policy_name"]).reset_index(drop=True)
    combined_all_policy.to_csv(OUTPUT_DIR / "combined_all_policy_summary.csv", index=False)

    new_env_ids = [spec["env_id"] for spec in NEW_ENV_SPECS]
    riccati_new = combined_all_policy[
        combined_all_policy["env_id"].isin(new_env_ids) & (combined_all_policy["policy_name"] == "riccati_reference")
    ][["env_id", "group", "tier", "mean_discounted_loss"]].rename(columns={"mean_discounted_loss": "riccati_loss"})
    env_best = combined_all_policy[combined_all_policy["env_id"].isin(new_env_ids)].groupby("env_id", as_index=False)["mean_discounted_loss"].min().rename(columns={"mean_discounted_loss": "best_policy_loss"})
    ...
    lines = []
    lines.append("# Phase 11 极端扩展矩阵")
    lines.append("")
    lines.append("## 范围")
    lines.append("")
    lines.append("| 维度 | 内容 |")
    lines.append("|---|---|")
    lines.append("| 新增环境 | `nonlinear/asymmetric/zlb` 各新增 `very_strong` 与 `extreme` 两档 |")
    lines.append("| RL 算法 | `PPO`、`TD3`、`SAC` |")
    lines.append(f"| Seeds | `{SEEDS}` |")
    lines.append("")
    lines.append("## 新增两档 RL 汇总")
    lines.append("")
    ...
    lines.append("")
    lines.append("## 新增两档中 Riccati 相对最优策略差距")
    lines.append("")
    lines.append(riccati_gap.round(6).to_markdown(index=False))
    lines.append("")
    lines.append("## 五档合并后 RL 汇总")
    lines.append("")
    lines.append(combined_rl[["env_id", "group", "tier", "algo", "mean_discounted_loss"]].round(6).to_markdown(index=False))
    lines.append("")
    lines.append("## 说明")
    lines.append("")
    lines.append("- 本批次不改 `phase10`，全部输出独立放在 `phase11`。")
    lines.append("- `zlb_*` 继续按更紧的 `ELB-tightness` 解释。")
    lines.append("- 若新增两档中 `Riccati` 仍然是最优，则说明扭曲还不够，需要继续加大。")
